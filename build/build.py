...
if __name__ == '__main__':
    ...
    parser.add_option('--build-root',                       dest='build_root',   default='./build_polyview',            help='Root of the build and install')
    ...
    build = []
    build0 = [zlib, png, jpeg, tiff, qt]

    if len(args) == 0 or opt.dev:
        if arch.os == 'linux':
            build.extend([m4, libtool, autoconf, automake])
        build.extend([bzip2, pbzip2])
        if arch.os == 'linux':
            build.extend([chrpath])
        build.extend(build0)
        if not opt.dev:
            build.extend([polyview])
